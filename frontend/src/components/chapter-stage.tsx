...
function ChapterStageComponent({
  ...
  useEffect(() => {
    ...
    // ── IntersectionObserver: track which chapter is in viewport (50%+ visible) ──
    // Uses native window scroll, root=null means viewport.
    ...
    // ── GSAP animations (no-op shim safe, wrapped in try/catch) ──
    try {
      ...
      const ctx = gsap.context(() => {
        if (!reduce) {
          ...
          if (far) {
            gsap.fromTo(far, { yPercent: -4, scale: 1.02 }, {
  ...
  return (
    ...
    >
      <div className="relative h-screen w-full overflow-hidden flex items-center">
        ...
        <div className="relative z-30 flex h-full w-full items-center pl-4 sm:pl-8 md:pl-36 lg:pl-60 xl:pl-64 pr-4 sm:pr-8 pt-24 sm:pt-28 lg:pt-20 pb-24 lg:pb-16 overflow-y-auto lg:overflow-visible">
          <div
            className={cn(
