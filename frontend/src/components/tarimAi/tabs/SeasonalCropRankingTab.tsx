...
interface SeasonalCropRankingTabProps {
  parcelId: string;
}

export function SeasonalCropRankingTab({ parcelId }: SeasonalCropRankingTabProps) {
  const [data, setData] = useState<SeasonalCropRankingResponse | null>(null)
  ...
  const loadData = async () => {
    try {
      ...
      const ranking = await tarimAi.getSeasonalCropRanking(parcelId, params)
      setData(ranking)
    } catch (err: any) {
  ...
  const handleExport = (format: 'csv' | 'json' | 'excel') => {
    const url = tarimAi.getSeasonalCropRankingExportUrl(parcelId, format, topFilter > 0 ? topFilter : undefined)
  ...
  if (loading && !data) {
    return (
      <div className="flex justify-center items-center h-64">
        <Spinner size="lg" />
        <span className="ml-3 text-slate-500">Sıralama motoru çalışıyor...</span>
      </div>
    )
  }
      ...
      <div className="space-y-4 relative">
        ...
        {data?.results.length === 0 && (
          <div className="text-center p-12 bg-white rounded-lg border border-dashed border-slate-300">
            ...
            <p className="text-slate-500 mt-1">Seçili filtrelere uygun sezonluk ürün bulunmuyor.</p>
          </div>
        )}
      </div>
