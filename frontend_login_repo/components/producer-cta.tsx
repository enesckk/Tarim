...
export function ProducerCta({ onTabClick, onOpenModal }: ProducerCtaProps) {
  ...
  return (
    ...
    >
        ...
        <div className="hidden sm:flex items-center gap-3.5 relative w-full h-full">
          ...
          <div
            ...
            className={cn(
              ...
              isLight ? 'border-[#B8842F]' : 'border-[#D6AE5E]',
            )}
          />
          ...
          <div ref={dustRef} className="pointer-events-none absolute bottom-2 left-0 z-20 flex gap-2">
            <div className={cn('dust-particle w-2 h-2 rounded-full blur-[0.5px] opacity-0', isLight ? 'bg-[#B8842F]/50' : 'bg-[#D6AE5E]/50')} />
            <div className={cn('dust-particle w-2.5 h-2.5 rounded-full blur-[1px] opacity-0', isLight ? 'bg-[#B8842F]/40' : 'bg-[#D6AE5E]/40')} />
            <div className={cn('dust-particle w-1.5 h-1.5 rounded-full blur-[0.5px] opacity-0', isLight ? 'bg-[#B8842F]/60' : 'bg-[#D6AE5E]/60')} />
          </div>

          {/* Layer 4: Vector SVG Tractor */}
          <div
            ref={tractorRef}
            className="pointer-events-none absolute bottom-1 left-0 z-30 flex items-center justify-center will-change-transform scale-100 md:scale-110 drop-shadow-[0_0_16px_rgba(214,174,94,0.7)]"
          >
            <svg viewBox="0 0 84 50" className={cn('w-16 h-10 transition-colors duration-300', isLight ? 'text-[#B8842F]' : 'text-[#D6AE5E]')}>
              <path
              ...
              <defs>
                <linearGradient id="headlight-glow" x1="0%" y1="0%" x2="100%" y2="0%">
                  ...
                  <stop offset="100%" stopColor={isLight ? '#B8842F' : '#D6AE5E'} stopOpacity="0" />
                </linearGradient>
              </defs>
              ...
              <path
                ...
                stroke={isLight ? '#B8842F' : '#D6AE5E'}
                strokeWidth="2"
              />

              <path
                d="M44 20H70V28H44V20Z"
                fill={isLight ? '#B8842F' : '#D6AE5E'}
                stroke="#FFF"
                strokeWidth="1.5"
              />

              <path d="M60 20V8H64V20" stroke="#FFF" strokeWidth="2" fill={isLight ? '#B8842F' : '#D6AE5E'} />

              <path d="M20 16H34V10H20V16Z" fill={isLight ? '#B8842F' : '#D6AE5E'} fillOpacity="0.4" stroke={isLight ? '#B8842F' : '#D6AE5E'} strokeWidth="1" />

              <g className="wheel wheel-rear" style={{ transformOrigin: '26px 35px' }}>
                <circle cx="26" cy="35" r="11" fill={isLight ? '#F4F1EA' : '#0A0D0A'} stroke={isLight ? '#B8842F' : '#D6AE5E'} strokeWidth="2.5" />
                <circle cx="26" cy="35" r="7" fill="none" stroke={isLight ? '#B8842F' : '#D6AE5E'} strokeWidth="1.5" />
                <line x1="26" y1="24" x2="26" y2="46" stroke={isLight ? '#B8842F' : '#D6AE5E'} strokeWidth="2" />
                <line x1="15" y1="35" x2="37" y2="35" stroke={isLight ? '#B8842F' : '#D6AE5E'} strokeWidth="2" />
                <circle cx="26" cy="35" r="3.5" fill="#FFF" />
              </g>

              <g className="wheel wheel-front" style={{ transformOrigin: '63px 37px' }}>
                <circle cx="63" cy="37" r="7" fill={isLight ? '#F4F1EA' : '#0A0D0A'} stroke={isLight ? '#B8842F' : '#D6AE5E'} strokeWidth="2.5" />
                <line x1="63" y1="30" x2="63" y2="44" stroke={isLight ? '#B8842F' : '#D6AE5E'} strokeWidth="1.8" />
                <line x1="56" y1="37" x2="70" y2="37" stroke={isLight ? '#B8842F' : '#D6AE5E'} strokeWidth="1.8" />
      ...
      {/* Right Controls: Compact Theme Switcher & Application Button */}
      <div className="pointer-events-auto flex items-center gap-1.5 sm:gap-2.5 shrink-0">
        ...
        <button
          ...
          className={cn(
            'flex items-center gap-1 px-2 sm:px-3.5 py-1 sm:py-1.5 rounded-full border backdrop-blur-[12px] transition-button-180 hover:scale-[1.02] active:scale-[0.99] shadow-sm text-xs font-medium',
            isLight
        ...
        <button
          ...
          className={cn(
            'group inline-flex items-center gap-1 sm:gap-2 rounded-full border px-2.5 sm:px-4 py-1 sm:py-1.5 text-[9.5px] sm:text-[11px] font-sans font-extrabold tracking-[0.1em] sm:tracking-[0.16em] backdrop-blur-[12px] transition-button-180 hover:scale-[1.02] active:scale-[0.99] shadow-sm whitespace-nowrap',
            isLight
